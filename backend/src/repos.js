...
// backend/src/repos.js (mock version)

async function getUserByEmail(email) {
  // Return a fake user
  if (email === 'admin@uwo.ca') {
    return { user_id: '1', email: 'admin@uwo.ca', full_name: 'Admin User' }
  }
  return { user_id: '2', email, full_name: 'Student User', faculty: 'SCIENCE' }
}
...
// you can leave other repo functions empty for now
async function createBallot() { return 'mock-ballot-id' }
async function getBallot() { return { ballot_id: 'mock-ballot-id' } }

module.exports = { getUserByEmail, listRoles, createBallot, getBallot }